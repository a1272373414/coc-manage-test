...
/**
 * Stream 操作工具类，集中处理 null safety。 使用方式： <pre>{@code
...
public final class StreamUtils {
	...
	public static <T, R> List<R> mapNonNull(Iterable<T> source, Function<? super T, ? extends R> mapper) {
		...
		return stream(source).map(mapper).filter(Objects::nonNull).collect(Collectors.toList());
	}
	...
	public static <T, R> Set<R> mapNonNullToSet(Iterable<T> source, Function<? super T, ? extends R> mapper) {
		...
		return stream(source).map(mapper).filter(Objects::nonNull).collect(Collectors.toSet());
	}
	...
	private static <T> Stream<T> streamNullable(Iterable<T> source) {
		return source instanceof java.util.Collection ? ((java.util.Collection<T>) source).stream()
