...
public class LeagueController extends BaseCrudController<League> {
	...
	/**
	 * 新增联赛： 1) 校验联赛编号唯一性 2) 插入联赛记录 3) 查询当前用户所属群组下的所有部落，为每个部落生成一条联赛部落成绩记录
	 */
	@Override
	@PostMapping
	public ApiResponse create(@RequestBody League body) {
		...
		if (body.getLeagueNo() != null && !body.getLeagueNo().isEmpty()) {
			League existing = leagueMapper
				.selectOne(new QueryWrapper<League>().eq("league_no", body.getLeagueNo()).eq("group_no", groupNo));
			if (existing != null)
				return ApiResponse.error(409, "该群组下联赛编号已存在：" + body.getLeagueNo());
		}
		mapper().insert(body);
	...
	public ApiResponse update(@RequestBody League body) {
		...
		if (body.getLeagueNo() != null && !body.getLeagueNo().isEmpty()) {
			...
			if (groupNo == null || groupNo.isEmpty()) {
				...
				if (self != null)
					groupNo = self.getGroupNo();
			}
			QueryWrapper<League> qw = new QueryWrapper<League>().eq("league_no", body.getLeagueNo())
				.ne("id", body.getId());
			if (groupNo != null && !groupNo.isEmpty()) {
			...
			if (existing != null)
				return ApiResponse.error(409, "该群组下联赛编号已被其他联赛使用：" + body.getLeagueNo());
		}
