...
public class SysMenuController extends BaseCrudController<SysMenu> {
   ...
   * 数据结构：[{ id, parentId, menuName, menuType, permission, path, sort, children: [...] }]
   * 健壮性：能处理 parentId 为 null/0/自身/指向不存在节点 等各种边界情况，
   *        这些节点都会被作为 root 显示，不会丢失。
   * 排序：selectList 已按 sort+id 排序，但 HashMap.values() 遍历顺序不稳定，
   *      需在挂载后对 roots 和每层 children 显式按 sort 升序排序（null 视为最大排最后）。
   */
  @GetMapping("/tree")
  ...
  /**
   * 级联删除菜单及其所有后代，并同步清理 sys_role_menu 中的关联。
   * 用法：DELETE /api/sys/menu/cascade/{id}
   * 必要性：sys_menu.parent_id 没有外键 ON DELETE CASCADE，常规 DELETE 只删单条会留下孤儿节点，
   *          sys_role_menu(role_id, menu_id) 同样没有外键，也需要清理避免历史数据指向已删菜单。
   * 策略：先按 parent_id 索引递归收集所有后代 id（含自身），在事务内删除所有菜单 + 关联的角色绑定。
   */
  ...
  public ApiResponse deleteCascade(@PathVariable Long id) {
    ...
    while (!stack.isEmpty()) {
      Long cur = stack.pop();
      if (cur == null || !toDelete.add(cur)) continue;
      List<Long> kids = childrenMap.get(cur);
      if (kids != null) {
        for (Long k : kids) stack.push(k);
      }
    }
