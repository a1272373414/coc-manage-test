...
public class SysRoleController extends BaseCrudController<SysRole> {
	...
	public ApiResponse getRoleMenus(@PathVariable Long id) {
		List<SysRoleMenu> list = roleMenuMapper.selectList(new QueryWrapper<SysRoleMenu>().eq("role_id", id));
		List<Long> menuIds = StreamUtils.mapNonNull(list, SysRoleMenu::getMenuId);
		return ApiResponse.ok(menuIds);
	}

	/**
	 * 设置角色的菜单集合：先物理删除再插入，全量替换。 menuIds 为 null 或空数组表示清空角色全部菜单。 注意：必须使用
	 * physicalDeleteByRoleId 绕过 MyBatis-Plus 逻辑删除拦截， 否则旧记录未被真正删除会导致 uk_role_menu 唯一索引冲突。
	 */
