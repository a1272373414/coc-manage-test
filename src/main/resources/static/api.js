...
(function () {
  ...
  const http = axios.create({
    baseURL: API_BASE,
    timeout: CONFIG.requestTimeout || 20000,
  });
  ...
  const api = {
    ...
    assignMenus: (roleId, menuIds) =>
      http.post("/api/sys/role/" + roleId + "/menus", menuIds),
    ...
    leagueImportPreview: (formData) =>
      http.post("/api/league/record/import/preview", formData, {
        headers: { "Content-Type": "multipart/form-data" },
      }),
    leagueImportConfirm: (body) =>
      http.post("/api/league/record/import/confirm", body),
    leagueImportTemplate: (type) =>
      http.get("/api/league/record/import/template", {
        params: { type: type },
        responseType: "blob",
      }),
    ...
    clanMemberImportPreview: (formData) =>
      http.post("/api/clan/member/import/preview", formData, {
        headers: { "Content-Type": "multipart/form-data" },
      }),
    clanMemberImportConfirm: (body) =>
    ...
    quickGroup: (groupNo) =>
      http.get("/api/quick/groups/" + encodeURIComponent(groupNo)),
    quickClans: (groupNo) =>
      http.get("/api/quick/clans", { params: { groupNo } }),
    quickLeagues: (groupNo) =>
      http.get("/api/quick/leagues", { params: { groupNo } }),
