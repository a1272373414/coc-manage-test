...
/**
 * AuthService 单元测试：使用 Mockito Mock 所有 Mapper 依赖， 不启动 Spring 上下文，不依赖数据库。
 */
...
class AuthServiceTest {
  ...
  void login_userNotFound() {
	...
	void login_disabled() {
		...
		RuntimeException ex = assertThrows(RuntimeException.class, () -> authService.login("disabled", "123456"));
	...
	void login_wrongPassword() {
		...
		RuntimeException ex = assertThrows(RuntimeException.class, () -> authService.login("admin", "wrong-password"));
	...
	void toAuthUser_withRolesAndPermissions() {
		...
		when(userRoleMapper.selectList(any(QueryWrapper.class))).thenReturn(Collections.singletonList(userRole));
		...
		when(roleMenuMapper.selectList(any(QueryWrapper.class))).thenReturn(Collections.singletonList(roleMenu));
	...
	void info_menusTopLevelSortedBySort() {
		...
		when(menuMapper.selectList(any(QueryWrapper.class))).thenReturn(Collections.emptyList())
	...
	void info_menusChildrenSortedBySort() {
		...
		when(menuMapper.selectList(any(QueryWrapper.class))).thenReturn(Collections.singletonList(pSystem))
	...
	void info_menuNodeCarriesSortField() {
		...
		when(userRoleMapper.selectList(any(QueryWrapper.class))).thenReturn(Collections.singletonList(userRole));
		when(roleMapper.selectBatchIds(anyCollection())).thenReturn(Collections.singletonList(role));
		when(roleMenuMapper.selectList(any(QueryWrapper.class))).thenReturn(Collections.singletonList(rm));
		when(menuMapper.selectList(any(QueryWrapper.class))).thenReturn(Collections.emptyList())
	...
	void info_sortNullMenusAppearLast() {
		...
		when(userRoleMapper.selectList(any(QueryWrapper.class))).thenReturn(Collections.singletonList(userRole));
		when(roleMapper.selectBatchIds(anyCollection())).thenReturn(Collections.singletonList(role));
		when(roleMenuMapper.selectList(any(QueryWrapper.class))).thenReturn(Arrays.asList(rm1, rm2));
		when(menuMapper.selectList(any(QueryWrapper.class))).thenReturn(Collections.emptyList())
