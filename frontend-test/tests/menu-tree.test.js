...
describe('MenuTree - 菜单树视图工厂', function () {
  ...
  describe('模板渲染 - path/permission 前缀处理', function () {
    ...
    test('path 字段已含 "/"，模板不应再加 "/" 前缀（避免出现 //system 双斜杠）', function () {
      ...
      expect(comp.template).not.toMatch(>\s*\/\s*\{\{\s*data\.path\s*\}\}\s*<);
      // 正确形式：直接用 data.path
    ...
    test('path 渲染示例 - 后端返回 "/system" 应显示为 "/system" 不是 "//system"', function () {
      // 通过纯文本模拟渲染验证：
      function renderPath(path) { return path; }
  ...
  describe('模板 - 必填标识', function () {
    ...
    test('路由路径有 required 标识（目录和菜单类型必填）', function () {
      expect(comp.template).toContain('label="路由路径" v-if="form.menuType !== 2" required');
    });

    test('权限标识有 required 标识（菜单和按钮类型必填）', function () {
      expect(comp.template).toContain('label="权限标识" v-if="form.menuType === 1 || form.menuType === 2" required');
    });
  });
  ...
  describe('filteredData - 关键字过滤', function () {
    ...
    function buildTree() {
      return [
        {
          id: 10, parentId: 0, menuName: '系统管理', path: '/system',
          menuType: 0,
          children: [
            { id: 11, parentId: 10, menuName: '用户管理', path: '/sys/user', permission: 'user:list', menuType: 1, children: [] },
            { id: 12, parentId: 10, menuName: '角色管理', path: '/sys/role', permission: 'role:list', menuType: 1, children: [] }
          ]
        },
        {
          id: 20, parentId: 0, menuName: '联赛管理', path: '/league', menuType: 0,
          children: [
            { id: 21, parentId: 20, menuName: '报名管理', path: '/league/signup', permission: 'league:signup', menuType: 1, children: [] }
          ]
        },
        { id: 30, parentId: 0, menuName: '数据看板', path: '/dashboard', menuType: 1, children: [] }
      ];
    }
    ...
    test('深层级过滤 - 三级嵌套仍正确保留父链', function () {
      ...
      // 添加三级菜单
      ctx.treeData = [{
        id: 1, parentId: 0, menuName: '系统管理', path: '/sys', menuType: 0,
        children: [{
          id: 2, parentId: 1, menuName: '权限管理', path: '/perm', menuType: 0,
          children: [
            { id: 3, parentId: 2, menuName: '用户列表', path: '/perm/users', menuType: 1, children: [] }
          ]
        }]
      }];
      ctx.keyword = '用户列表';
  ...
  describe('onSave() - 表单校验', function () {
    ...
    function validate(form, ElementPlus) {
      if (!form.menuName) {
        if (ElementPlus && ElementPlus.ElMessage) ElementPlus.ElMessage.warning('请输入菜单名称');
        return { ok: false, message: '请输入菜单名称' };
      }
      if (form.menuType !== 2 && !form.path) {
        if (ElementPlus && ElementPlus.ElMessage) ElementPlus.ElMessage.warning('请输入路由路径');
        return { ok: false, message: '请输入路由路径' };
      }
      if ((form.menuType === 1 || form.menuType === 2) && !form.permission) {
        if (ElementPlus && ElementPlus.ElMessage) ElementPlus.ElMessage.warning('请输入权限标识');
    ...
    test('按钮类型菜单只要求菜单名和权限标识', function () {
      var form = { menuName: '新增按钮', menuType: 2, permission: 'sys:btn:add' };
      expect(validate(form).ok).toBe(true);
    });

    test('菜单类型菜单要求菜单名 + 路径 + 权限标识', function () {
      var form = { menuName: '用户管理', menuType: 1, path: '/sys/user', permission: 'user:list' };
      expect(validate(form).ok).toBe(true);
    });
  ...
  describe('loadTree() - 异步加载', function () {
    ...
    test('成功时将返回数据赋给 treeData', async function () {
      ...
      var treeData = [
        { id: 1, parentId: 0, menuName: '顶级菜单', path: '/top', menuType: 1, children: [] }
      ];
      // stub COC.api.get
    ...
    test('返回非数组时回退为空数组', async function () {
      ...
      var originalGet = window.COC.api.get;
      window.COC.api.get = jest.fn().mockResolvedValue({ unexpected: 'object' });
