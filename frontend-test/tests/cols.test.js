...
describe('cols.js - 列配置', function () {
  ...
  describe('列配置完整性', function () {
    ...
    test('ID 列统一配置 hideInForm 和 hideInTable', function () {
      var groups = [
        COC_COLS.clanCols, COC_COLS.memberCols, COC_COLS.warCols,
        COC_COLS.warRecordCols, COC_COLS.leagueCols, COC_COLS.leagueRecordCols,
        COC_COLS.leagueSignupCols, COC_COLS.groupCols, COC_COLS.menuCols,
        COC_COLS.dictGroupCols, COC_COLS.dictItemCols
      ];
      groups.forEach(function (cols) {
        var idCol = cols.find(function (c) { return c.prop === 'id'; });
        if (idCol) {
  ...
  describe('memberCols - 部落成员列', function () {
    ...
    test('成员编号为非必填（无 rule）', function () {
      var col = COC_COLS.memberCols.find(function (c) { return c.prop === 'memberNo'; });
      expect(col.rule).toBeUndefined();
    });

    test('包含在组状态字段（select 类型，已加入/已退出）', function () {
      var col = COC_COLS.memberCols.find(function (c) { return c.prop === 'memberStatus'; });
    ...
    test('参战状态为 switch 类型且无自定义文字（默认参战状态）', function () {
      var col = COC_COLS.memberCols.find(function (c) { return c.prop === 'warStatus'; });
    ...
    test('部落编号为远程下拉选择（remote-select）', function () {
      var col = COC_COLS.memberCols.find(function (c) { return c.prop === 'clanNo'; });
  ...
  describe('clanCols - 部落列', function () {
    ...
    test('包含部落名称和编号搜索字段', function () {
      var nameCol = COC_COLS.clanCols.find(function (c) { return c.prop === 'clanName'; });
      expect(nameCol.search).toBe(true);
      expect(nameCol.rule).toBeDefined();

      var noCol = COC_COLS.clanCols.find(function (c) { return c.prop === 'clanNo'; });
      expect(noCol.search).toBe(true);
      expect(noCol.rule).toBeDefined();
    });

    test('简介为 textarea 类型', function () {
      var introCol = COC_COLS.clanCols.find(function (c) { return c.prop === 'intro'; });
  ...
  describe('leagueCols - 联赛列', function () {
    ...
    test('升降级为 select 类型并有选项', function () {
      var col = COC_COLS.leagueCols.find(function (c) { return c.prop === 'promoteStatus'; });
      expect(col.type).toBe('select');
      expect(col.options).toEqual([
        { label: '无', value: 0 },
        { label: '晋升', value: 1 },
    ...
    test('联赛段位为字典下拉（dictCode=league_tier）', function () {
      var col = COC_COLS.leagueCols.find(function (c) { return c.prop === 'tier'; });
    ...
    test('报名开始/结束为 date 类型', function () {
      var startCol = COC_COLS.leagueCols.find(function (c) { return c.prop === 'signupStart'; });
      expect(startCol.type).toBe('date');
      var endCol = COC_COLS.leagueCols.find(function (c) { return c.prop === 'signupEnd'; });
  ...
  describe('menuCols - 菜单列', function () {
    ...
    test('父菜单为 tree-select 类型', function () {
      var col = COC_COLS.menuCols.find(function (c) { return c.prop === 'parentId'; });
    ...
    test('菜单类型为 select 并有目录/菜单/按钮选项', function () {
      var col = COC_COLS.menuCols.find(function (c) { return c.prop === 'menuType'; });
      expect(col.type).toBe('select');
      expect(col.options).toEqual([
        { label: '目录', value: 0 },
        { label: '菜单', value: 1 },
  ...
  describe('dictItemCols - 字典项列', function () {
    ...
    test('所属分组为 select 类型且 options 为函数', function () {
      var col = COC_COLS.dictItemCols.find(function (c) { return c.prop === 'groupCode'; });
      expect(col.type).toBe('select');
    ...
    test('状态为 switch 类型', function () {
      var col = COC_COLS.dictItemCols.find(function (c) { return c.prop === 'status'; });
      expect(col.type).toBe('switch');
  ...
  describe('groupCols - 群组列', function () {
    ...
    test('群主为 remote-select 类型', function () {
      var col = COC_COLS.groupCols.find(function (c) { return c.prop === 'ownerId'; });
      expect(col.type).toBe('remote-select');
    ...
    test('群主名称仅在表格显示（hideInForm）', function () {
      var col = COC_COLS.groupCols.find(function (c) { return c.prop === 'ownerName'; });
