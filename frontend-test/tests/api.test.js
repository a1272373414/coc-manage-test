...
describe("api.js - 前端接口层", function () {
  ...
  describe("COC.api - 接口封装", function () {
    ...
    test("page 调用 GET {url}/page 带参数", function () {
      ...
      expect(spy).toHaveBeenCalledWith("/api/clan/page", {
        params: { keyword: "test", current: 1 },
      });
      spy.mockRestore();
    });
    ...
    test("dictItems 请求字典项带 size=9999", function () {
      ...
      expect(spy).toHaveBeenCalledWith("/api/dict/item/page", {
        params: { groupCode: "war_status", size: 9999 },
      });
      spy.mockRestore();
    });
  });
  ...
  describe("响应拦截器", function () {
    ...
    test("code=401 时清除 token 并 reject", async function () {
      ...
      await expect(interceptor.onFulfilled(mockResponse)).rejects.toThrow(
        "登录已过期",
      );
    ...
    test("网络错误时 reject", async function () {
      ...
      await expect(interceptor.onRejected(networkError)).rejects.toBe(
        networkError,
      );
    });
