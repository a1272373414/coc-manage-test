...
describe('ClanCrud - 部落群组编号自动填充', function () {
  ...
  describe('clanCols - groupNo 列配置', function () {
    ...
    test('包含 groupNo 列定义', function () {
      var groupNoCol = COC_COLS.clanCols.find(function (c) { return c.prop === 'groupNo'; });
    ...
    test('groupNo 包含在 formCols 中（submit 时会提交）', function () {
      ...
      var formCols = cols.filter(function (c) { return !c.hideInForm; });
      var groupNoInForm = formCols.find(function (c) { return c.prop === 'groupNo'; });
  ...
  describe('openCreate() - 自动填充 groupNo', function () {
    ...
    test('ElMessage.warning 在 groupNo 为空时被调用', function () {
      ...
      ElementPlus.ElMessage.warning = function (msg) { warningCalled = msg; };
      var ctx = { form: null, dialogVisible: false };
      clanOpenCreate(ctx);
  ...
  describe('submit() - groupNo 包含在提交数据中', function () {
    ...
    test('新增部落时 payload 包含 groupNo', function () {
      ...
      var formCols = COC_COLS.clanCols.filter(function (c) { return !c.hideInForm; });
      var form = { clanName: '部落A', clanNo: 'C001', groupNo: 'G001', intro: '简介' };
      var payload = {};
  ...
  describe('源代码关键逻辑验证', function () {
    ...
    function readSrc(filename) {
      return fs.readFileSync(
        path.resolve(__dirname, '..', 'src', 'main', 'resources', 'static', filename),
        'utf8'
      );
    }
