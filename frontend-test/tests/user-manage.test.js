...
describe('UserManage - 用户管理组件', function () {
  ...
  describe('openCreate() - 新增用户密码默认 123456', function () {
    ...
    test('新增用户表单 password 字段默认为 123456', function () {
      // 模拟 openCreate 的逻辑
      var form = { username: '', nickname: '', phone: '', email: '', status: 1, password: '123456' };
  ...
  describe('submit() - 用户名必填校验', function () {
    ...
    test('用户名有值时校验通过', function () {
      var result = validateAndClean({ username: 'admin', password: '123456', nickname: '管理员' });
    ...
    test('编辑模式密码为空时从 payload 中删除', function () {
      var result = validateAndClean({ id: 1, username: 'admin', password: '', nickname: '管理员' });
      expect(result.ok).toBe(true);
      expect(result.payload.password).toBeUndefined();
    });

    test('编辑模式密码有值时保留在 payload 中', function () {
      var result = validateAndClean({ id: 1, username: 'admin', password: 'newpass', nickname: '管理员' });
      expect(result.ok).toBe(true);
  ...
  describe('源代码关键逻辑验证', function () {
    ...
    var appSrc = fs.readFileSync(
      path.resolve(__dirname, '..', 'src', 'main', 'resources', 'static', 'app.js'),
      'utf8'
    );
    ...
    test('openEdit 中 password 默认为空字符串', function () {
      ...
      expect(appSrc).toMatch(/Object\.assign\(\{\},\s*row,\s*\{\s*password:\s*''\s*\}\)/);
    });
    ...
    test('submit 中密码为空时从 payload 删除', function () {
      expect(appSrc).toContain("if (!payload.password) delete payload.password");
    });
