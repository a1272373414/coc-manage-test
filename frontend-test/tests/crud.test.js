...
describe('crud.js - CRUD 组件工厂', function () {
  ...
  beforeEach(function () {
    ...
    var testCols = [
      ...
      { prop: 'name', label: '名称', search: true, rule: [{ required: true, message: '请输入名称', trigger: 'blur' }] },
      { prop: 'status', label: '状态', type: 'switch', activeText: '启用', inactiveText: '禁用', default: 1 },
      { prop: 'count', label: '数量', type: 'number', default: 0 },
      { prop: 'category', label: '分类', type: 'select', options: [{ label: 'A', value: 'a' }, { label: 'B', value: 'b' }] },
      { prop: 'intro', label: '简介', type: 'textarea' }
    ];

    // 创建 CRUD 组件定义
    crudComp = createCrud({ name: 'TestCrud', baseUrl: '/api/test', cols: testCols });
  });
  ...
  describe('computed 属性', function () {
    ...
    test('searchCols - 过滤出可搜索列', function () {
      ...
      var searchCols = typeof ctx.searchCols === 'function'
    ...
    test('tableCols - 过滤出表格显示列（排除 hideInTable）', function () {
      ...
      var tableCols = typeof ctx.tableCols === 'function'
        ? ctx.tableCols.call(ctx)
        : ctx.tableCols;
      // id 有 hideInTable，其余无
      expect(tableCols.length).toBe(5);
      expect(tableCols.find(function (c) { return c.prop === 'id'; })).toBeUndefined();
    });
    ...
    test('formCols - 过滤出表单字段（排除 hideInForm）', function () {
      ...
      var formCols = typeof ctx.formCols === 'function'
        ? ctx.formCols.call(ctx)
        : ctx.formCols;
      // id 有 hideInForm
      expect(formCols.length).toBe(5);
      expect(formCols.find(function (c) { return c.prop === 'id'; })).toBeUndefined();
    });
  });
  ...
  describe('defaultForm()', function () {
    ...
    test('switch 类型默认值为 1', function () {
      var ctx = Object.assign(crudComp.data(), {
        cols: crudComp.data().cols,
        formCols: crudComp.data().cols.filter(function (c) { return !c.hideInForm; })
      });
  ...
  describe('optionsFor()', function () {
    ...
    test('select 类型返回 c.options', function () {
      ...
      var catCol = ctx.cols.find(function (c) { return c.prop === 'category'; });
      var opts = crudComp.methods.optionsFor.call(ctx, catCol);
      expect(opts).toEqual([{ label: 'A', value: 'a' }, { label: 'B', value: 'b' }]);
    });
    ...
    test('options 为函数时调用函数返回结果', function () {
      ...
      var col = { prop: 'test', options: function () { return [{ label: 'X', value: 'x' }]; } };
  ...
  describe('openCreate()', function () {
    ...
    test('设置 dialogTitle 为"新增"并打开对话框', function () {
      var ctx = Object.assign(crudComp.data(), {
        cols: crudComp.data().cols,
        formCols: crudComp.data().cols.filter(function (c) { return !c.hideInForm; }),
        formColsFilter: crudComp.data().cols.filter(function (c) { return !c.hideInForm; }),
        remoteOptions: {},
        remoteSearch: function () {},
        defaultForm: crudComp.methods.defaultForm,
        openCreate: crudComp.methods.openCreate,
        $nextTick: function (cb) { if (cb) cb(); },
        $refs: {}
      });
      // 绑定 formCols 计算属性
      ctx.formCols = crudComp.data().cols.filter(function (c) { return !c.hideInForm; });
  ...
  describe('openEdit()', function () {
    ...
    test('设置 dialogTitle 为"编辑"并填充表单', function () {
      var row = { id: 42, name: '测试', status: 0, count: 5, category: 'b', intro: '备注' };
      var ctx = Object.assign(crudComp.data(), {
        cols: crudComp.data().cols,
        formCols: crudComp.data().cols.filter(function (c) { return !c.hideInForm; }),
        remoteOptions: {},
    ...
    test('switch 类型 null 值转为 1', function () {
      var row = { id: 1, name: 'test', status: null, count: 0, category: '', intro: '' };
      var ctx = Object.assign(crudComp.data(), {
        cols: crudComp.data().cols,
        formCols: crudComp.data().cols.filter(function (c) { return !c.hideInForm; }),
        remoteOptions: {},
